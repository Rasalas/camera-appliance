async (page) => {
  // Open /system/allgemein?case=success|download|copy|restart|lost-response|navigation first.
  ...
  if (!['success', 'download', 'copy', 'restart', 'lost-response', 'navigation'].includes(scenario)) throw Error('Unknown scenario');
  ...
  await page.reload();
  if (scenario === 'navigation') await page.getByRole('link', { name: 'Version und Updates', exact: true }).click();
  const downloadButton = scenario === 'navigation' ? page.getByRole('main').getByRole('button', { name: 'Update herunterladen', exact: true }) : page.getByRole('button', { name: 'Update herunterladen', exact: true });
  ...
  const installButton = scenario === 'navigation' ? page.getByRole('main').getByRole('button', { name: 'Jetzt aktualisieren', exact: true }) : page.getByRole('button', { name: 'Update jetzt installieren', exact: true });
  await installButton.waitFor({ timeout: 60000 });
  ...
  if (scenario === 'navigation') await page.route('**/api/system/update/install', async route => {
    // Leave the dedicated page while its install request is in flight. Let
    // the real server install after navigation, without mocking its response.
