...
func TestEnsureSingleFlightBlocksConcurrentApply(t *testing.T) {
	release, err := EnsureSingleFlight()
	if err != nil {
		t.Fatalf("first reservation should succeed: %v", err)
	}
	if _, busy := EnsureSingleFlight(); busy == nil {
		t.Fatal("second concurrent reservation must fail")
	}
	release()
	if release2, err := EnsureSingleFlight(); err != nil {
