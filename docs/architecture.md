...
| `snapshotupload` | Upload destinations, crops, per-camera filenames/directories, JPEG preparation, FTP/SFTP transfer and interval/quiet-hour scheduling | One `Service` shares the upload lock and persisted file settings between manual and scheduled work. Empty camera directories use the global destination; overrides use remote POSIX paths, relative to the server login directory or absolute. Transfers publish completed temporary files via rename, using the POSIX extension for SFTP replacement. The daemon starts `Run(ctx)` once; durable due times and attempt state live in dotted SQLite settings. `RunDue` has an injected clock and uses stored camera credentials/crops. Tests cover restart, quiet hours/DST and loopback FTP/SFTP, including two cameras in separate directories, existing destinations and failed replacement. Passwords belong to `secrets`. |
