...
| `snapshotupload` | Upload destinations, crops, JPEG preparation, FTP/SFTP transfer and per-camera interval/quiet-hour scheduling | One `Service` shares the upload lock between manual and scheduled work. The daemon starts `Run(ctx)` once; durable due times and attempt state live in dotted SQLite settings. `RunDue` has an injected clock and uses stored camera credentials/crops. Tests cover restart, quiet hours/DST and loopback FTP/SFTP. Passwords belong to `secrets`. |
