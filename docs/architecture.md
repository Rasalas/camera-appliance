...
| `snapshotupload` | Upload destinations, crops, per-camera filenames, JPEG preparation, FTP/SFTP transfer and interval/quiet-hour scheduling | One `Service` shares the upload lock and persisted naming policy between manual and scheduled work. Transfers publish completed temporary files via rename, using the POSIX extension for SFTP replacement. The daemon starts `Run(ctx)` once; durable due times and attempt state live in dotted SQLite settings. `RunDue` has an injected clock and uses stored camera credentials/crops. Tests cover restart, quiet hours/DST and loopback FTP/SFTP, including existing destinations and failed replacement. Passwords belong to `secrets`. |
